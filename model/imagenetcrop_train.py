    ...
    def __init__(self, xs=(3, 32, 32), nz=1, zchannels=16, nprocessing=1, kernel_size=3, resdepth=2,
        ...
        if root_process:
            ...
            log_dir = os.path.join(
                'runs/imagenetcrop/', current_time + '_' + socket.gethostname() + tag)
        ...
        # the scale parameter of the bottom (zi = 1) generative model is modelled conditional
    ...
    def generate(self, i):
        ...
        def distribution(given):
            ...
            if i == 0:
                ...
                scale = ((2. / 255.) / 8.) + modules.softplus(self.gen_std(h))
...
def test(model, device, epoch, ema, data_loader, tag, root_process):
    ...
    if root_process:
        ...
        if elbo < model.best_elbo and not np.isnan(elbo):
            ...
            if not os.path.exists(f'params/imagenetcrop/'):
                os.makedirs(f'params/imagenetcrop/')
            torch.save(model.state_dict(), f'params/imagenetcrop/{tag}')
            torch.save(model.state_dict(), f'params/imagenetcrop/epoch{epoch}_{tag}')
            print("saved params\n")
...
if __name__ == '__main__':
    ...
    transform_ops = transforms.Compose([transforms.RandomCrop(32), transforms.ToTensor(), ToInt()])
    ...
    train_set = modules.ImageNet(root='data/imagenetfull/train', file='train.npy', transform=transform_ops)
    test_set = modules.ImageNet(root='data/imagenetfull/test', file='test.npy', transform=transform_ops)
